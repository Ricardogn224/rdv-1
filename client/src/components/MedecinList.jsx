import React from 'react';

function MedecinList() {
  return (
    <section className="flex mt-40 pration-trouver">
      <div className="zone-info-praticien flex-column">

        <div className="flex-center">
          <img src="./icone.png" alt="people" />
        </div>

        <p>
          <b className="nom-medecin">Sandrine IRIGOYEN</b>
        </p>

        <p>
          <b className="poste">Chirurgien-dentiste</b>
        </p>

        <div className="flex flex-row flex-center">
          <svg className="svg-icon" style={{ width: '1em', height: '1em', verticalAlign: 'middle', fill: 'black', overflow: 'hidden' }} viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
            <path d="M918.613333 305.066667a42.666667 42.666667 0 0 0-42.666666 0L725.333333 379.306667A128 128 0 0 0 597.333333 256H213.333333a128 128 0 0 0-128 128v256a128 128 0 0 0 128 128h384a128 128 0 0 0 128-123.306667l151.893334 75.946667A42.666667 42.666667 0 0 0 896 725.333333a42.666667 42.666667 0 0 0 22.613333-6.4A42.666667 42.666667 0 0 0 938.666667 682.666667V341.333333a42.666667 42.666667 0 0 0-20.053334-36.266666zM640 640a42.666667 42.666667 0 0 1-42.666667 42.666667H213.333333a42.666667 42.666667 0 0 1-42.666666-42.666667V384a42.666667 42.666667 0 0 1 42.666666-42.666667h384a42.666667 42.666667 0 0 1 42.666667 42.666667z m213.333333-26.453333l-128-64v-75.093334l128-64z" />
          </svg>
          <p className="video-p">Consultation vidéo disponible</p>
        </div>

        <div className="adresse">
          <p>4 Allée du Dr Lejzer Ludwik Zamenhof</p>
          <p>31100 Toulouse</p>
        </div>

      </div>

      <div className="planning">
                        <div className=" flex space-between zone-calendrier">
                            <div>
                                <p>
                                </p>
                            </div>
                            <div className="day">
                                <p>MAR</p>
                                <p>11 JUL</p>
                            </div>
                            <div className="day">
                                <p>MER</p>
                                <p>12 JUL</p>
                            </div>
                            <div className="day">
                                <p>JEU</p>
                                <p>13 JUL</p>
                            </div>
                            <div className="day">
                                <p>VEN</p>
                                <p>14 JUL</p>
                            </div>
                            <div className="day">
                                <p>SAM</p>
                                <p>15 JUL</p>
                            </div>
                            <div className="day">
                                <p>DIM</p>
                                <p>16 JUL</p>
                            </div>
                            <div className="day">
                                <p>></p>
                            </div>

                        </div>
                     
                        <div className=" flex space-between ">
                            <div>
                                <p>
                                </p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <div className="dispo-hour">
                                    <p className="hour">09:20</p>
                                </div>

                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p></p>
                            </div>

                        </div>
                       
                        <div className=" flex space-between ">
                            <div>
                                <p>
                                </p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <div className="dispo-hour">
                                    <p className="hour">09:20</p>
                                </div>

                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p></p>
                            </div>

                        </div>
                        
                        <div className=" flex space-between ">
                            <div>
                                <p>
                                </p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <div className="dispo-hour">
                                    <p className="hour">09:20</p>
                                </div>

                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p></p>
                            </div>

                        </div>
                        
                        <div className=" flex space-between ">
                            <div>
                                <p>
                                </p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <div className="dispo-hour">
                                    <p className="hour">09:20</p>
                                </div>

                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p>-</p>
                            </div>
                            <div className="day">
                                <p></p>
                            </div>

                        </div>
                        
      </div>

    </section>
  );
}

export default MedecinList;
